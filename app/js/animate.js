$(document).ready(function(){
  ...
  // }
    function animateFirstLoadingPage() {
      tl.fromTo( $(".heading"), 0.5, { y: -300},{y: 0},1 )
        .from( $(".title__text > h1"), 0.5, { opacity: 0, ease: Bounce.easeOut , y: -100 , x:40}, "+=0.2" )
        .from( $(".title__text > p"), 0.5, { opacity: 0, x: -200, ease: Back.easeOut.config(0.7)} )
        .staggerFromTo($(".title__mac > .mac"), 0.5,{ opacity: 0, scale: 0}, {opacity: 1, scale: 1}, "+0.3")
        .from( $(".navigation__partner"), 0.5, { opacity: 0, y:300 } )
        .from( $(".navigation__advertiser"), 0.5, { opacity: 0, y:300 });
    }
  ...
  //     .to( $(".navigation__partner"), 1.5, { rotation: -80 ,y:-2500, display: "none", ease: Power3.easeOut }, "-=1.9" )
  //     .to( $(".navigation__advertiser"), 1, { rotation: 80 ,y:-2500, display: "none" ,ease: Power3.easeOut }, "-=1" )
  ...
  // }
    ...
    function transitionAnimationFurther() {
      tl.to( $(".mac__layer1"), 1, { rotation: 60 ,y:-1100 ,display: 'none'}, "mac" )
        .to( $(".mac__layer0"), 0.8, { rotation: 60 ,y:-1100, display: 'none'},"mac" )
        .to( $(".navigation__partner"), 0.5, { rotation: -80 ,y:-2500, display: "none", ease: Power3.easeOut }, "-=1.9" )
        .to( $(".navigation__advertiser"), 0.1, { rotation: 80 ,y:-2500, display: "none" ,ease: Power3.easeOut }, "-=1" )
        .to( $(".heading__menu"), 0.1, { y: -500 , display: "none", ease: Linear.easeNone },"-=0.5" )
        .to( $(".title__text > h1"), 0.1, { y: -800 , display: "none", ease: Linear.easeNone}, "-=0.5"  )
        .to( $(".title__text > p"), 0.8, { y: 1200, display: 'none', ease: Linear.easeNone},"-=1")
        .to( $(".mac__layer2,.mac__layer3,.mac__layer4 "), 1, { animationIterationCount: 0, y: 1300 ,display: 'none', ease:Power3.easeOut },"-=0.5")
        .to( $(".heading__logo"), 1, { top: 40+"vh", left: 35+"vw", position: "absolute" , ease: Linear.easeNone} );
    }
